/*
  Usage:
    pnpm -s tsx scripts/concurrency-call-tool.ts <concurrency> <toolName> '<argsJson>' [durationSec]
  Examples:
    pnpm -s tsx scripts/concurrency-call-tool.ts 70 get_agile_boards '{}'
    pnpm -s tsx scripts/concurrency-call-tool.ts 70 search '{"jql":"project = PROJ AND status = Open"}' 30
*/
...
async function callTool(hubKey: string, id: number, toolName: string, args: any): Promise<boolean> {
...
async function main() {
    ...
    const toolName = process.argv[3] || 'get_agile_boards';
    const argsJson = process.argv[4] || '{}';
    const durationSec = parseInt(process.argv[5] || '0', 10);
    let args: any;
    try { args = JSON.parse(argsJson); } catch { throw new Error('Invalid argsJson'); }
    ...
    const runBatch = async (): Promise<{ ok: number; total: number }> => {
        ...
        for (let i = 1; i <= concurrency; i += 1) {
            promises.push(callTool(hubKey, i, toolName, args).catch(() => false));
        }
    ...
    if (durationSec > 0) {
        ...
        let rounds = 0; let okSum = 0; let totalSum = 0;
        while (Date.now() < end) {
            const { ok, total } = await runBatch();
            rounds += 1; okSum += ok; totalSum += total;
