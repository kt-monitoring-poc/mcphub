...
export class DebugLogger {
  ...
  static logNetworkRequest(requestId: string, method: string, url: string, headers?: any, body?: any) {
    ...
    if (headers) {
      ...
      console.log(chalk.cyan(`   Headers: ${JSON.stringify(safeHeaders, null, 2)}`));
    }
    
    if (body) {
      const safeBody = typeof body === 'string' ? body.substring(0, 200) + '...' : JSON.stringify(body, null, 2).substring(0, 200) + '...';
  ...
  static logNetworkResponse(requestId: string, statusCode: number, responseTime: number, responseData?: any) {
    ...
    if (responseData) {
      const safeResponse = typeof responseData === 'string' 
        ? responseData.substring(0, 200) + '...' 
        : JSON.stringify(responseData, null, 2).substring(0, 200) + '...';
      console.log(chalk.cyan(`   Response: ${safeResponse}`));
    }
  }
